...
class FacialKeyPointsDataset(Dataset):
    def __init__(self, csv_file_path, split='training',model_input_size= 224 ,device=torch.device('cpu')):
        super().__init__()
    ...
    def get_img(self, index):  # to get img from  training folder using the keypoints df
        ...
        img = np.asarray(
            img) / 255.0  # range of pixel value is between 0(black) and 255(white), we normalize it to [0,1]
    ...
    def get_keypoints(self, index, original_size):
        kp = self.df.iloc[index, 1:].to_numpy().astype(
            np.float32)  # since kp_x,y is list ,we cant divide it by int . So, we use broadcasting concept of numpy
...
if __name__ == '__main__':
    ...
    training_data = FacialKeyPointsDataset(csv_file_path=configuration['train_data_csv_path'],device=device)
